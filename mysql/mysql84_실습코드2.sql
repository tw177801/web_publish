...
**************************************************/    
-- HRD 부서의 사원 ID, 사원명, 부서명, 연봉 
-- SYS 부서의 사원 ID, 사원명, 부서명, 연봉 실행 결과 합치기 
...
-- SYS 부서의 사원들의 ID, 사원명, 부서 ID, PHONE, 연봉 
-- UNION ALL (중복허용, 전체 데이터 모두 출력): 22 
-- UNION (중복 허용 X, 전체 데이터 모두 출력): 17
SELECT COUNT(*)
	FROM (
		SELECT EMP_ID, EMP_NAME, DEPT_ID, PHONE, SALARY -- 16 
			FROM EMPLOYEE 
...
SELECT 	ROW_NUMBER() OVER(ORDER BY SUM(SALARY) DESC) AS NO,
...
SELECT ROW_NUMBER() OVER(ORDER BY YEAR) AS NO,
	YEAR, DEPT_ID, SALARY
FROM (SELECT NO, YEAR, DEPT_ID, SALARY 
        ...
        GROUP BY YEAR, DEPT_ID
        ORDER BY YEAR, SUM(SALARY) DESC) T1
        WHERE NO = 1
UNION 
SELECT NO, YEAR, DEPT_ID, SALARY 
		FROM (SELECT ROW_NUMBER() OVER(ORDER BY SUM(SALARY) DESC) AS NO,
		LEFT(HIRE_DATE, 4) AS YEAR, 
		DEPT_ID,
		FORMAT(SUM(SALARY),0) AS SALARY
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	- 삭제: DROP VIEW [VIEW명]
++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
-- VIEW는 SQL을 통해 생성되므로 INFORMATION_SCHEMA라는 사전에 등록이 됨 
...
-- ALTER VIEW VIEW_SUM_SALARY
-- AS
-- SELECT ROW_NUMBER() OVER(ORDER BY YEAR) AS NO,
-- 	CONCAT((YEAR),'년도') AS 년도, DEPT_ID AS 부서별ID, CONCAT((SALARY),'만원') AS 연봉
-- FROM (SELECT NO, YEAR, DEPT_ID, SALARY 
-- 		FROM (SELECT ROW_NUMBER() OVER(ORDER BY SUM(SALARY) DESC) AS NO,
-- 		LEFT(HIRE_DATE, 4) AS YEAR, 
-- 		DEPT_ID,
